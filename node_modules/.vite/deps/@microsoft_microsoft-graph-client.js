...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/RequestMethod.js
/**
* -------------------------------------------------------------------------------------------
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/content/BatchRequestContent.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
*/
var BatchRequestContent = class BatchRequestContent {
	/**
	* @private
	* @static
	...
	static validateDependencies(requests) {
		const isParallel = (reqs) => {
			...
			while (!cur.done) {
				...
				if (curReq.dependsOn !== void 0 && curReq.dependsOn.length > 0) return false;
				cur = iterator.next();
			}
			return true;
		};
		const isSerial = (reqs) => {
			...
			if (firstRequest.dependsOn !== void 0 && firstRequest.dependsOn.length > 0) return false;
			let prev = cur;
			cur = iterator.next();
			while (!cur.done) {
				const curReq = cur.value[1];
				if (curReq.dependsOn === void 0 || curReq.dependsOn.length !== 1 || curReq.dependsOn[0] !== prev.value[1].id) return false;
				prev = cur;
				cur = iterator.next();
			}
			return true;
		...
		const isSame = (reqs) => {
			...
			const firstRequest = cur.value[1];
			let dependencyId;
			if (firstRequest.dependsOn === void 0 || firstRequest.dependsOn.length === 0) dependencyId = firstRequest.id;
			else if (firstRequest.dependsOn.length === 1) {
				const fDependencyId = firstRequest.dependsOn[0];
				if (fDependencyId !== firstRequest.id && reqs.has(fDependencyId)) dependencyId = fDependencyId;
				else return false;
			} else return false;
			cur = iterator.next();
			while (!cur.done) {
				const curReq = cur.value[1];
				if ((curReq.dependsOn === void 0 || curReq.dependsOn.length === 0) && dependencyId !== curReq.id) return false;
				if (curReq.dependsOn !== void 0 && curReq.dependsOn.length !== 0) {
					if (curReq.dependsOn.length === 1 && (curReq.id === dependencyId || curReq.dependsOn[0] !== dependencyId)) return false;
					if (curReq.dependsOn.length > 1) return false;
				}
				cur = iterator.next();
			}
			return true;
		...
		if (requests.size === 0) {
			const error = /* @__PURE__ */ new Error("Empty requests map, Please provide at least one request.");
	...
	*/
	static getRequestData(request) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			requestData.url = (/* @__PURE__ */ new RegExp("^https?://")).test(request.url) ? "/" + request.url.split(/.*?\/\/.*?\//)[1] : request.url;
			requestData.method = request.method;
			const headers = {};
			request.headers.forEach((value, key) => {
				headers[key] = value;
			});
			if (Object.keys(headers).length) requestData.headers = headers;
			if (request.method === RequestMethod.PATCH || request.method === RequestMethod.POST || request.method === RequestMethod.PUT) requestData.body = yield BatchRequestContent.getRequestBody(request);
			/**
				...
				if (typeof Blob !== "undefined") {
					...
					body = yield new Promise((resolve) => {
						reader.addEventListener("load", () => {
							...
							resolve((/* @__PURE__ */ new RegExp("^s*data:(.+?/.+?(;.+?=.+?)*)?(;base64)?,(.*)s*$")).exec(dataURL)[4]);
						}, false);
						reader.readAsDataURL(blob);
					});
				} else if (typeof Buffer !== "undefined") body = (yield request.buffer()).toString("base64");
				bodyParsed = true;
			} catch (e) {}
			return body;
		});
	...
	constructor(requests) {
		...
		if (typeof requests !== "undefined") {
			...
			if (requests.length > limit) {
				const error = /* @__PURE__ */ new Error(`Maximum requests limit exceeded, Max allowed number of requests are ${limit}`);
	...
	*/
	addRequest(request) {
		const limit = BatchRequestContent.requestLimit;
		if (request.id === "") {
			const error = /* @__PURE__ */ new Error(`Id for a request is empty, Please provide an unique id`);
		...
		if (this.requests.has(request.id)) {
			const error = /* @__PURE__ */ new Error(`Adding request with duplicate id ${request.id}, Make the id of the requests unique`);
		...
		*/
		while (!cur.done) {
			...
			if (typeof dependencies !== "undefined") {
				...
				if (dependencies.length === 0) delete cur.value[1].dependsOn;
			}
			cur = iterator.next();
		}
		return deleteStatus;
	...
	*/
	getContent() {
		return __awaiter(this, void 0, void 0, function* () {
			...
			let cur = iterator.next();
			if (cur.done) {
				const error = /* @__PURE__ */ new Error("No requests added yet, Please add at least one request.");
				error.name = "Empty Payload";
				throw error;
			}
			if (!BatchRequestContent.validateDependencies(this.requests)) {
				const error = /* @__PURE__ */ new Error(`Invalid dependency found, Dependency should be:
1. Parallel - no individual request states a dependency in the dependsOn property.
2. Serial - all individual requests depend on the previous individual request.
3. Same - all individual requests that state a dependency in the dependsOn property, state the same dependency.`);
			...
			while (!cur.done) {
				...
				*/
				if (batchRequestData.body !== void 0 && (batchRequestData.headers === void 0 || batchRequestData.headers["content-type"] === void 0)) {
					const error = /* @__PURE__ */ new Error(`Content-type header is not mentioned for request #${requestStep.id}, For request having body, Content-type header should be mentioned`);
					error.name = "Invalid Content-type header";
					throw error;
				}
				batchRequestData.id = requestStep.id;
				if (requestStep.dependsOn !== void 0 && requestStep.dependsOn.length > 0) batchRequestData.dependsOn = requestStep.dependsOn;
	...
	*/
	addDependency(dependentId, dependencyId) {
		if (!this.requests.has(dependentId)) {
			const error = /* @__PURE__ */ new Error(`Dependent ${dependentId} does not exists, Please check the id`);
			error.name = "Invalid Dependent";
			throw error;
		}
		if (typeof dependencyId !== "undefined" && !this.requests.has(dependencyId)) {
			const error = /* @__PURE__ */ new Error(`Dependency ${dependencyId} does not exists, Please check the id`);
			error.name = "Invalid Dependency";
			throw error;
		}
		if (typeof dependencyId !== "undefined") {
			const dependent = this.requests.get(dependentId);
			if (dependent.dependsOn === void 0) dependent.dependsOn = [];
			if (dependent.dependsOn.indexOf(dependencyId) !== -1) {
				const error = /* @__PURE__ */ new Error(`Dependency ${dependencyId} is already added for the request ${dependentId}`);
			...
			if (typeof prev !== "undefined") {
				...
				if (cur.value[1].dependsOn === void 0) cur.value[1].dependsOn = [];
				if (cur.value[1].dependsOn.indexOf(dId) !== -1) {
					const error = /* @__PURE__ */ new Error(`Dependency ${dId} is already added for the request ${dependentId}`);
					error.name = "Duplicate Dependency";
					throw error;
				}
				cur.value[1].dependsOn.push(dId);
			} else {
				const error = /* @__PURE__ */ new Error(`Can't add dependency ${dependencyId}, There is only a dependent request in the batch`);
	...
	*/
	removeDependency(dependentId, dependencyId) {
		...
		if (typeof request === "undefined" || request.dependsOn === void 0 || request.dependsOn.length === 0) return false;
		if (typeof dependencyId !== "undefined") {
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/content/BatchResponseContent.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
var BatchResponseContent = class {
	...
	*/
	createResponseObject(responseJSON) {
		...
		if (responseJSON.statusText !== void 0) options.statusText = responseJSON.statusText;
	...
	*/
	update(response) {
		...
		for (let i = 0, l = responses.length; i < l; i++) this.responses.set(responses[i].id, this.createResponseObject(responses[i]));
	}
	/**
	* @public
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/Constants.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/GraphClientError.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
*/
var GraphClientError = class GraphClientError extends Error {
	/**
	* @public
	* @static
	* @async
	...
	*/
	static setGraphClientError(error) {
		...
		if (error instanceof Error) graphClientError = error;
		else {
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/GraphRequestUtil.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
var urlJoin = (urlSegments) => {
	...
	return Array.prototype.slice.call(urlSegments).reduce(joiner);
};
/**
...
var serializeContent = (content) => {
	...
	if (className === "Buffer" || className === "Blob" || className === "File" || className === "FormData" || typeof content === "string") return content;
	if (className === "ArrayBuffer") content = Buffer.from(content);
	else if (className === "Int8Array" || className === "Int16Array" || className === "Int32Array" || className === "Uint8Array" || className === "Uint16Array" || className === "Uint32Array" || className === "Uint8ClampedArray" || className === "Float32Array" || className === "Float64Array" || className === "DataView") content = Buffer.from(content.buffer);
	else try {
...
var isCustomHostValid = (host) => {
	if (host.indexOf("/") !== -1) throw new GraphClientError("Please add only hosts or hostnames to the CustomHosts config. If the url is `http://example.com:3000/`, host is `example:3000`");
};
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/middleware/MiddlewareControl.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/middleware/MiddlewareUtil.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
var getRequestHeader = (request, options, key) => {
	let value = null;
	if (typeof Request !== "undefined" && request instanceof Request) value = request.headers.get(key);
	else if (typeof options !== "undefined" && options.headers !== void 0) {
		if (typeof Headers !== "undefined" && options.headers instanceof Headers) value = options.headers.get(key);
		else if (options.headers instanceof Array) {
			const headers = options.headers;
			for (let i = 0, l = headers.length; i < l; i++) if (headers[i][0] === key) {
				value = headers[i][1];
				break;
			}
		} else if (options.headers[key] !== void 0) value = options.headers[key];
	}
	return value;
};
/**
...
var setRequestHeader = (request, options, key, value) => {
	if (typeof Request !== "undefined" && request instanceof Request) request.headers.set(key, value);
	else if (typeof options !== "undefined") if (options.headers === void 0) options.headers = new Headers({ [key]: value });
	else if (typeof Headers !== "undefined" && options.headers instanceof Headers) options.headers.set(key, value);
	else if (options.headers instanceof Array) {
		let i = 0;
		...
		if (i === l) options.headers.push([key, value]);
	} else Object.assign(options.headers, { [key]: value });
};
/**
* @constant
...
var appendRequestHeader = (request, options, key, value) => {
	if (typeof Request !== "undefined" && request instanceof Request) request.headers.append(key, value);
	else if (typeof options !== "undefined") if (options.headers === void 0) options.headers = new Headers({ [key]: value });
	else if (typeof Headers !== "undefined" && options.headers instanceof Headers) options.headers.append(key, value);
	else if (options.headers instanceof Array) options.headers.push([key, value]);
	else if (options.headers === void 0) options.headers = { [key]: value };
	else if (options.headers[key] === void 0) options.headers[key] = value;
	else options.headers[key] += `, ${value}`;
};
/**
* @constant
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/middleware/options/AuthenticationHandlerOptions.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/middleware/options/TelemetryHandlerOptions.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
*/
var TelemetryHandlerOptions = class TelemetryHandlerOptions {
	constructor() {
		/**
		* @private
	...
	*/
	static updateFeatureUsageFlag(context, flag) {
		...
		else context.middlewareControl = new MiddlewareControl();
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/middleware/AuthenticationHandler.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
*/
var AuthenticationHandler = class AuthenticationHandler {
	/**
	* @public
	* @constructor
	...
	execute(context) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			if (isGraphURL(url) || context.customHosts && isCustomHost(url, context.customHosts)) {
				let options;
				if (context.middlewareControl instanceof MiddlewareControl) options = context.middlewareControl.getMiddlewareOptions(AuthenticationHandlerOptions);
				...
				if (!authenticationProvider) authenticationProvider = this.authenticationProvider;
			...
			} else if (context.options.headers) delete context.options.headers[AuthenticationHandler.AUTHORIZATION_HEADER];
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/middleware/HTTPMessageHandler.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/middleware/options/RetryHandlerOptions.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
*/
var RetryHandlerOptions = class RetryHandlerOptions {
	/**
	* @public
	* @constructor
	...
	*/
	constructor(delay = RetryHandlerOptions.DEFAULT_DELAY, maxRetries = RetryHandlerOptions.DEFAULT_MAX_RETRIES, shouldRetry = RetryHandlerOptions.defaultShouldRetry) {
		if (delay > RetryHandlerOptions.MAX_DELAY && maxRetries > RetryHandlerOptions.MAX_MAX_RETRIES) {
			const error = /* @__PURE__ */ new Error(`Delay and MaxRetries should not be more than ${RetryHandlerOptions.MAX_DELAY} and ${RetryHandlerOptions.MAX_MAX_RETRIES}`);
			error.name = "MaxLimitExceeded";
			throw error;
		} else if (delay > RetryHandlerOptions.MAX_DELAY) {
			const error = /* @__PURE__ */ new Error(`Delay should not be more than ${RetryHandlerOptions.MAX_DELAY}`);
			error.name = "MaxLimitExceeded";
			throw error;
		} else if (maxRetries > RetryHandlerOptions.MAX_MAX_RETRIES) {
			const error = /* @__PURE__ */ new Error(`MaxRetries should not be more than ${RetryHandlerOptions.MAX_MAX_RETRIES}`);
			error.name = "MaxLimitExceeded";
			throw error;
		} else if (delay < 0 && maxRetries < 0) {
			const error = /* @__PURE__ */ new Error(`Delay and MaxRetries should not be negative`);
			error.name = "MinExpectationNotMet";
			throw error;
		} else if (delay < 0) {
			const error = /* @__PURE__ */ new Error(`Delay should not be negative`);
			error.name = "MinExpectationNotMet";
			throw error;
		} else if (maxRetries < 0) {
			const error = /* @__PURE__ */ new Error(`MaxRetries should not be negative`);
			error.name = "MinExpectationNotMet";
			throw error;
		}
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/middleware/RetryHandler.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
var RetryHandler = class RetryHandler {
	...
	*/
	isBuffered(request, options) {
		...
		if (method === RequestMethod.PUT || method === RequestMethod.PATCH || method === RequestMethod.POST) {
			if (getRequestHeader(request, options, "Content-Type") === "application/octet-stream") return false;
		}
		return true;
	}
	/**
	...
	*/
	getDelay(response, retryAttempts, delay) {
		...
		let newDelay;
		if (retryAfter !== null) if (Number.isNaN(Number(retryAfter))) newDelay = Math.round((new Date(retryAfter).getTime() - Date.now()) / 1e3);
		else newDelay = Number(retryAfter);
		else newDelay = retryAttempts >= 2 ? this.getExponentialBackOffTime(retryAttempts) + delay + getRandomness() : delay + getRandomness();
	...
	*/
	...
	getOptions(context) {
		let options;
		if (context.middlewareControl instanceof MiddlewareControl) options = context.middlewareControl.getMiddlewareOptions(this.options.constructor);
		if (typeof options === "undefined") options = Object.assign(new RetryHandlerOptions(), this.options);
		return options;
	}
	/**
	* @private
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/middleware/options/RedirectHandlerOptions.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
*/
var RedirectHandlerOptions = class RedirectHandlerOptions {
	/**
	* @public
	* @constructor
	...
	*/
	constructor(maxRedirects = RedirectHandlerOptions.DEFAULT_MAX_REDIRECTS, shouldRedirect = RedirectHandlerOptions.defaultShouldRedirect) {
		if (maxRedirects > RedirectHandlerOptions.MAX_MAX_REDIRECTS) {
			const error = /* @__PURE__ */ new Error(`MaxRedirects should not be more than ${RedirectHandlerOptions.MAX_MAX_REDIRECTS}`);
			error.name = "MaxLimitExceeded";
			throw error;
		}
		if (maxRedirects < 0) {
			const error = /* @__PURE__ */ new Error(`MaxRedirects should not be negative`);
			error.name = "MinExpectationNotMet";
			throw error;
		}
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/middleware/RedirectHandler.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
var RedirectHandler = class RedirectHandler {
	...
	*/
	shouldDropAuthorizationHeader(requestUrl, redirectUrl) {
		...
		if (requestMatches !== null) requestAuthority = requestMatches[0];
		const redirectMatches = schemeHostRegex.exec(redirectUrl);
		if (redirectMatches !== null) redirectAuthority = redirectMatches[0];
	...
	*/
	getOptions(context) {
		let options;
		if (context.middlewareControl instanceof MiddlewareControl) options = context.middlewareControl.getMiddlewareOptions(RedirectHandlerOptions);
		if (typeof options === "undefined") options = Object.assign(new RedirectHandlerOptions(), this.options);
		return options;
	}
	/**
	* @private
		...
		return __awaiter(this, void 0, void 0, function* () {
			...
			if (redirectCount < options.maxRedirects && this.isRedirect(response) && this.hasLocationHeader(response) && options.shouldRedirect(response)) {
				...
				if (response.status === RedirectHandler.STATUS_CODE_SEE_OTHER) {
					...
					if (!this.isRelativeURL(redirectUrl) && this.shouldDropAuthorizationHeader(response.url, redirectUrl)) delete context.options.headers[RedirectHandler.AUTHORIZATION_HEADER];
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/Version.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/middleware/TelemetryHandler.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
*/
var TelemetryHandler = class TelemetryHandler {
	/**
	* @public
	* @async
	* To execute the current middleware
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/middleware/MiddlewareFactory.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/middleware/options/ChaosStrategy.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/middleware/options/ChaosHandlerOptions.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
var ChaosHandlerOptions = class {
	...
	*/
	constructor(chaosStrategy = ChaosStrategy.RANDOM, statusMessage = "Some error Happened", statusCode, chaosPercentage, responseBody, headers) {
		...
		if (this.chaosPercentage > 100) throw new Error("Error Pecentage can not be more than 100");
	}
};
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/middleware/options/ChaosHandlerData.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/middleware/ChaosHandler.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
var ChaosHandler = class {
	...
	*/
	createResponseHeaders(chaosHandlerOptions, requestID, requestDate) {
		...
		if (chaosHandlerOptions.statusCode === 429) responseHeader.append("retry-after", "3");
	...
	*/
	createResponseBody(chaosHandlerOptions, requestID, requestDate) {
		if (chaosHandlerOptions.responseBody) return chaosHandlerOptions.responseBody;
		let body;
		if (chaosHandlerOptions.statusCode >= 400) body = { error: {
			code: httpStatusCode[chaosHandlerOptions.statusCode],
			message: chaosHandlerOptions.statusMessage,
			innerError: {
	...
	*/
	sendRequest(chaosHandlerOptions, context) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			if (chaosHandlerOptions.chaosStrategy === ChaosStrategy.MANUAL && !this.nextMiddleware || Math.floor(Math.random() * 100) < chaosHandlerOptions.chaosPercentage) this.createResponse(chaosHandlerOptions, context);
			else if (this.nextMiddleware) yield this.nextMiddleware.execute(context);
		});
	}
	/**
	...
	*/
	getRelativeURL(urlMethod) {
		...
		let relativeURL;
		if (pattern.exec(urlMethod) !== null) relativeURL = pattern.exec(urlMethod)[1];
		return relativeURL;
	}
	/**
		...
		if (chaosHandlerOptions.chaosStrategy === ChaosStrategy.MANUAL) {
			if (chaosHandlerOptions.statusCode === void 0) {
				...
				if (this.manualMap.get(relativeURL) !== void 0) {
					if (this.manualMap.get(relativeURL).get(requestMethod) !== void 0) chaosHandlerOptions.statusCode = this.manualMap.get(relativeURL).get(requestMethod);
				} else this.manualMap.forEach((value, key) => {
					if (new RegExp(key + "$").test(relativeURL)) {
						if (this.manualMap.get(key).get(requestMethod) !== void 0) chaosHandlerOptions.statusCode = this.manualMap.get(key).get(requestMethod);
					}
				});
			}
		} else chaosHandlerOptions.statusCode = this.getRandomStatusCode(requestMethod);
	}
	/**
	* To get the options for execution of the middleware
	* @private
	...
	*/
	getOptions(context) {
		let options;
		if (context.middlewareControl instanceof MiddlewareControl) options = context.middlewareControl.getMiddlewareOptions(ChaosHandlerOptions);
		if (typeof options === "undefined") options = Object.assign(new ChaosHandlerOptions(), this.options);
		return options;
	}
	/**
	* To execute the current middleware
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/ResponseType.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/GraphResponseHandler.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
*/
var GraphResponseHandler = class GraphResponseHandler {
	/**
	* @private
	* @static
	...
	*/
	static parseDocumentResponse(rawResponse, type) {
		if (typeof DOMParser !== "undefined") return new Promise((resolve, reject) => {
			rawResponse.text().then((xmlString) => {
				try {
					resolve(new DOMParser().parseFromString(xmlString, type));
				} catch (error) {
					reject(error);
				}
			});
		});
		else return Promise.resolve(rawResponse.body);
	}
	/**
	* @private
	* @static
	...
	*/
	static convertResponse(rawResponse, responseType) {
		return __awaiter(this, void 0, void 0, function* () {
			if (rawResponse.status === 204) return Promise.resolve();
			let responseValue;
			...
			switch (responseType) {
				...
				default:
					if (contentType !== null) {
						...
						if (new RegExp(ContentTypeRegexStr.DOCUMENT).test(mimeType)) responseValue = yield GraphResponseHandler.parseDocumentResponse(rawResponse, mimeType);
						else if (new RegExp(ContentTypeRegexStr.IMAGE).test(mimeType)) responseValue = rawResponse.blob();
						else if (mimeType === ContentType.TEXT_PLAIN) responseValue = yield rawResponse.text();
						else if (mimeType === ContentType.APPLICATION_JSON) responseValue = yield rawResponse.json();
						else responseValue = Promise.resolve(rawResponse.body);
					} else
 /**
	...
	*/
	static getResponse(rawResponse, responseType, callback) {
		return __awaiter(this, void 0, void 0, function* () {
			if (responseType === ResponseType.RAW) return Promise.resolve(rawResponse);
			else {
				const response = yield GraphResponseHandler.convertResponse(rawResponse, responseType);
				if (rawResponse.ok) if (typeof callback === "function") callback(null, response);
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/tasks/FileUploadTask/Range.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/tasks/FileUploadTask/UploadResult.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
*/
var UploadResult = class UploadResult {
	/**
	* @public
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/tasks/LargeFileUploadTask.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
var LargeFileUploadTask = class {
	...
	*/
	constructor(client, file, uploadSession, options = {}) {
		...
		*/
		...
		this.client = client;
		if (!file.sliceFile) throw new GraphClientError("Please pass the FileUpload object, StreamUpload object or any custom implementation of the FileObject interface");
		else this.file = file;
		this.file = file;
		if (!options.rangeSize) options.rangeSize = this.DEFAULT_FILE_SIZE;
		this.options = options;
	...
	*/
	getNextRange() {
		if (this.nextRange.minValue === -1) return this.nextRange;
		const minVal = this.nextRange.minValue;
		let maxValue = minVal + this.options.rangeSize - 1;
		if (maxValue >= this.file.size) maxValue = this.file.size - 1;
	...
	*/
	sliceFile(range) {
		...
		if (this.file.content instanceof ArrayBuffer || this.file.content instanceof Blob || this.file.content instanceof Uint8Array) return this.file.content.slice(range.minValue, range.maxValue + 1);
	...
	*/
		...
		return __awaiter(this, void 0, void 0, function* () {
			...
			while (!this.uploadSession.isCancelled) {
				...
				if (nextRange.maxValue === -1) {
					const err = /* @__PURE__ */ new Error("Task with which you are trying to upload is already completed, Please check for your uploaded file");
				...
				if (!rawResponse) throw new GraphClientError("Something went wrong! Large file upload slice response is null.");
	...
	reportProgress(uploadEventHandlers, nextRange) {
		if (uploadEventHandlers && uploadEventHandlers.progress) uploadEventHandlers.progress(nextRange, uploadEventHandlers.extraCallbackParam);
	}
	/**
	* @public
	* @async
	...
	*/
	cancel() {
		return __awaiter(this, void 0, void 0, function* () {
			...
			if (cancelResponse.status === 204) this.uploadSession.isCancelled = true;
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/tasks/FileUploadTask/FileObjectClasses/FileUpload.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
var FileUpload = class {
	...
	*/
	constructor(content, name, size) {
		...
		this.name = name;
		this.size = size;
		if (!content || !name || !size) throw new GraphClientError("Please provide the upload content, name of the file and size of the file");
	}
	/**
	* @public
	* Slices the file content to the given range
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/tasks/OneDriveLargeFileUploadTaskUtil.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
var roundTo320KB = (value) => {
	if (value > 320 * 1024) value = Math.floor(value / (320 * 1024)) * 320 * 1024;
	return value;
};
/**
* @constant
...
var getValidRangeSize = (rangeSize = DEFAULT_FILE_SIZE) => {
	...
	if (rangeSize > sixtyMB) rangeSize = sixtyMB;
	return roundTo320KB(rangeSize);
};
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/tasks/OneDriveLargeFileUploadTask.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
*/
var OneDriveLargeFileUploadTask = class OneDriveLargeFileUploadTask extends LargeFileUploadTask {
	/**
	* @private
	* @static
	...
	*/
	static create(client, file, options) {
		return __awaiter(this, void 0, void 0, function* () {
			if (!client || !file || !options) throw new GraphClientError("Please provide the Graph client instance, file object and OneDriveLargeFileUploadOptions value");
	...
	*/
	static createTaskWithFileObject(client, fileObject, options) {
		return __awaiter(this, void 0, void 0, function* () {
			if (!client || !fileObject || !options) throw new GraphClientError("Please provide the Graph client instance, FileObject interface implementation and OneDriveLargeFileUploadOptions value");
			...
			return new OneDriveLargeFileUploadTask(client, fileObject, yield OneDriveLargeFileUploadTask.createUploadSession(client, requestUrl, uploadSessionPayload), {
				rangeSize: getValidRangeSize(options.rangeSize),
	...
	*/
	static createUploadSession(client, requestUrl, payloadOptions) {
		const _super = Object.create(null, { createUploadSession: { get: () => super.createUploadSession } });
		return __awaiter(this, void 0, void 0, function* () {
			const payload = { item: {
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/tasks/FileUploadTask/FileObjectClasses/StreamUpload.js
/**
* @class
...
var StreamUpload = class {
	constructor(content, name, size) {
		...
		this.name = name;
		this.size = size;
		if (!content || !name || !size) throw new GraphClientError("Please provide the Readable Stream content, name of the file and size of the file");
	}
	/**
	* @public
	* Slices the file content to the given range
		...
		return __awaiter(this, void 0, void 0, function* () {
			...
			*/
			if (this.previousSlice) {
				if (range.minValue < this.previousSlice.range.minValue) throw new GraphClientError("An error occurred while uploading the stream. Please restart the stream upload from the first byte of the file.");
				if (range.minValue < this.previousSlice.range.maxValue) {
					...
					if (range.minValue === previousRangeMin && range.maxValue === previousRangeMax) return this.previousSlice.fileSlice;
					/**
					...
					*/
					if (range.maxValue === previousRangeMax) return this.previousSlice.fileSlice.slice(range.minValue, range.maxValue + 1);
					/**
			...
			if (this.content && this.content.readable) if (this.content.readableLength >= rangeSize) bufs.push(this.content.read(rangeSize));
			else bufs.push(yield this.readNBytesFromStream(rangeSize));
			else throw new GraphClientError("Stream is not readable.");
	...
	*/
	readNBytesFromStream(size) {
		return new Promise((resolve, reject) => {
			...
			let length = 0;
			this.content.on("end", () => {
				if (remainder > 0) return reject(new GraphClientError("Stream ended before reading required range size"));
			});
			this.content.on("readable", () => {
				...
				*/
				...
				while (length < size && (chunk = this.content.read(remainder)) !== null) {
					length += chunk.length;
					chunks.push(chunk);
					if (remainder > 0) remainder = size - length;
				}
				if (length === size) return resolve(Buffer.concat(chunks));
				if (!this.content || !this.content.readable) return reject(new GraphClientError("Error encountered while reading the stream during the upload"));
			});
		});
	}
};
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/tasks/PageIterator.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
var PageIterator = class {
	...
	*/
	iterationHelper() {
		if (this.collection === void 0) return false;
		let advance = true;
	...
	fetchAndUpdateNextPageData() {
		return __awaiter(this, void 0, void 0, function* () {
			...
			if (this.requestOptions) {
				if (this.requestOptions.headers) graphRequest = graphRequest.headers(this.requestOptions.headers);
				if (this.requestOptions.middlewareOptions) graphRequest = graphRequest.middlewareOptions(this.requestOptions.middlewareOptions);
				if (this.requestOptions.options) graphRequest = graphRequest.options(this.requestOptions.options);
			}
	...
	*/
	iterate() {
		return __awaiter(this, void 0, void 0, function* () {
			...
			while (advance) if (this.nextLink !== void 0) {
				yield this.fetchAndUpdateNextPageData();
				advance = this.iterationHelper();
			} else advance = false;
			if (this.nextLink === void 0 && this.cursor >= this.collection.length) this.complete = true;
		});
	}
	/**
	* @public
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/CustomAuthenticationProvider.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
			...
			return new Promise((resolve, reject) => {
				this.provider((error, accessToken) => __awaiter(this, void 0, void 0, function* () {
					if (accessToken) resolve(accessToken);
						...
						if (!error) error = new GraphClientError("Access token is undefined or empty.						Please provide a valid token.						For more help - https://github.com/microsoftgraph/msgraph-sdk-javascript/blob/dev/docs/CustomAuthenticationProvider.md");
						reject(yield GraphClientError.setGraphClientError(error));
					}
				}));
			});
		});
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/GraphError.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
*/
var GraphError = class GraphError extends Error {
	/**
	* @public
	* @constructor
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/GraphErrorHandler.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
*/
var GraphErrorHandler = class GraphErrorHandler {
	/**
	* @private
	* @static
	...
	*/
	static constructError(error, statusCode, rawResponse) {
		...
		if (error.name !== void 0) gError.code = error.name;
	...
	*/
	static getError(error = null, statusCode = -1, callback, rawResponse) {
		return __awaiter(this, void 0, void 0, function* () {
			let gError;
			if (error && error.error) gError = GraphErrorHandler.constructErrorFromResponse(error, statusCode, rawResponse);
			else if (error instanceof Error) gError = GraphErrorHandler.constructError(error, statusCode, rawResponse);
			else {
				gError = new GraphError(statusCode);
				gError.body = error;
			}
			if (typeof callback === "function") callback(gError, null);
			else return gError;
		});
	}
};
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/GraphRequest.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
var GraphRequest = class {
	...
	constructor(httpClient, config, path) {
		...
		*/
		this.parsePath = (path) => {
			...
			if (queryStrPos === -1) this.urlComponents.path = path;
			else {
				this.urlComponents.path = path.substr(0, queryStrPos);
				const queryParams = path.substring(queryStrPos + 1, path.length).split("&");
				for (const queryParam of queryParams) this.parseQueryParameter(queryParam);
			}
		};
	...
	*/
	addCsvQueryParameter(propertyName, propertyValue, additionalProperties) {
		...
		let allValues = [];
		if (additionalProperties.length > 1 && typeof propertyValue === "string") allValues = Array.prototype.slice.call(additionalProperties);
		else if (typeof propertyValue === "string") allValues.push(propertyValue);
		else allValues = allValues.concat(propertyValue);
	...
	*/
	buildFullUrl() {
		...
		if (this.config.debugLogging) console.log(url);
		return url;
	}
	/**
	* @private
	...
	*/
	createQueryString() {
		...
		if (Object.keys(urlComponents.oDataQueryParams).length !== 0) {
			for (const property in urlComponents.oDataQueryParams) if (Object.prototype.hasOwnProperty.call(urlComponents.oDataQueryParams, property)) query.push(property + "=" + urlComponents.oDataQueryParams[property]);
		}
		if (Object.keys(urlComponents.otherURLQueryParams).length !== 0) {
			for (const property in urlComponents.otherURLQueryParams) if (Object.prototype.hasOwnProperty.call(urlComponents.otherURLQueryParams, property)) query.push(property + "=" + urlComponents.otherURLQueryParams[property]);
		}
		if (urlComponents.otherURLQueryOptions.length !== 0) for (const str of urlComponents.otherURLQueryOptions) query.push(str);
	...
	parseQueryParameter(queryDictionaryOrString) {
		if (typeof queryDictionaryOrString === "string") {
			...
			if (queryDictionaryOrString.indexOf("&") !== -1) {
				...
				for (const str of queryParams) this.parseQueryParamenterString(str);
			} else this.parseQueryParamenterString(queryDictionaryOrString);
		} else if (queryDictionaryOrString.constructor === Object) {
			for (const key in queryDictionaryOrString) if (Object.prototype.hasOwnProperty.call(queryDictionaryOrString, key)) this.setURLComponentsQueryParamater(key, queryDictionaryOrString[key]);
		}
		return this;
	}
	/**
	...
	*/
	parseQueryParamenterString(queryParameter) {
		...
		} else this.urlComponents.otherURLQueryOptions.push(queryParameter);
	}
	/**
	* @private
	...
	*/
	setURLComponentsQueryParamater(paramKey, paramValue) {
		...
		} else this.urlComponents.otherURLQueryParams[paramKey] = paramValue;
	}
	/**
	* @private
	...
	*/
	isValidQueryKeyValuePair(queryString) {
		...
		if (indexofFirstEquals === -1) return false;
		if (queryString.indexOf("(") !== -1 && queryString.indexOf("(") < indexofFirstEquals) return false;
		return true;
	}
	/**
	* @private
	...
	*/
	updateRequestOptions(options) {
		...
		if (options.headers !== void 0) Object.assign(optionsHeaders, options.headers);
	...
	send(request, options, callback) {
		...
		return __awaiter(this, void 0, void 0, function* () {
			...
			try {
				...
				let statusCode;
				if (rawResponse) statusCode = rawResponse.status;
	...
	*/
	setHeaderContentType() {
		...
		for (const headerKey of headerKeys) if (headerKey.toLowerCase() === "content-type") return;
		this.header("Content-Type", "application/json");
	}
	/**
	* @public
	...
	*/
	headers(headers) {
		for (const key in headers) if (Object.prototype.hasOwnProperty.call(headers, key)) this._headers[key] = headers[key];
		return this;
	}
	/**
	* @public
	...
	*/
	options(options) {
		for (const key in options) if (Object.prototype.hasOwnProperty.call(options, key)) this._options[key] = options[key];
		return this;
	}
	/**
	* @public
	...
	*/
	get(callback) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			const options = { method: RequestMethod.GET };
	...
	*/
	post(content, callback) {
		return __awaiter(this, void 0, void 0, function* () {
			...
			if ((content && content.constructor && content.constructor.name) === "FormData") options.headers = {};
			else {
	...
	*/
	delete(callback) {
		return __awaiter(this, void 0, void 0, function* () {
			const url = this.buildFullUrl();
			const options = { method: RequestMethod.DELETE };
			return yield this.send(url, options, callback);
		});
	}
	/**
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/HTTPClient.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
var HTTPClient = class {
	...
	*/
	setMiddleware(...middleware) {
		if (middleware.length > 1) this.parseMiddleWareArray(middleware);
		else this.middleware = middleware[0];
	}
	/**
	* @private
	...
	*/
	parseMiddleWareArray(middlewareArray) {
		middlewareArray.forEach((element, index) => {
			if (index < middlewareArray.length - 1) element.setNext(middlewareArray[index + 1]);
		});
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/HTTPClientFactory.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
*/
var HTTPClientFactory = class HTTPClientFactory {
	/**
	* @public
	* @static
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/ValidatePolyFilling.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
var validatePolyFilling = () => {
	if (typeof Promise === "undefined" && typeof fetch === "undefined") {
		const error = /* @__PURE__ */ new Error("Library cannot function without Promise and fetch. So, please provide polyfill for them.");
		error.name = "PolyFillNotAvailable";
		throw error;
	} else if (typeof Promise === "undefined") {
		const error = /* @__PURE__ */ new Error("Library cannot function without Promise. So, please provide polyfill for it.");
		error.name = "PolyFillNotAvailable";
		throw error;
	} else if (typeof fetch === "undefined") {
		const error = /* @__PURE__ */ new Error("Library cannot function without fetch. So, please provide polyfill for it.");
		error.name = "PolyFillNotAvailable";
		throw error;
	}
	return true;
...
//#endregion
//#region node_modules/@microsoft/microsoft-graph-client/lib/es/src/Client.js
/**
* -------------------------------------------------------------------------------------------
* Copyright (c) Microsoft Corporation.  All Rights Reserved.  Licensed under the MIT License.
* See License in the project root for license information.
...
var Client = class Client {
	...
	*/
	static init(options) {
		const clientOptions = {};
		for (const i in options) if (Object.prototype.hasOwnProperty.call(options, i)) clientOptions[i] = i === "authProvider" ? new CustomAuthenticationProvider(options[i]) : options[i];
	...
	*/
	constructor(clientOptions) {
		...
		*/
		...
		validatePolyFilling();
		for (const key in clientOptions) if (Object.prototype.hasOwnProperty.call(clientOptions, key)) this.config[key] = clientOptions[key];
		let httpClient;
		if (clientOptions.authProvider !== void 0 && clientOptions.middleware !== void 0) {
			...
			throw error;
		} else if (clientOptions.authProvider !== void 0) httpClient = HTTPClientFactory.createWithAuthenticationProvider(clientOptions.authProvider);
		else if (clientOptions.middleware !== void 0) httpClient = new HTTPClient(...[].concat(clientOptions.middleware));
		else {
			const error = /* @__PURE__ */ new Error();
			error.name = "InvalidMiddlewareChain";
...
//#endregion
export { AuthenticationHandler, AuthenticationHandlerOptions, BatchRequestContent, BatchResponseContent, ChaosHandler, ChaosHandlerOptions, ChaosStrategy, Client, CustomAuthenticationProvider, FeatureUsageFlag, FileUpload, GraphClientError, GraphError, GraphRequest, HTTPMessageHandler, LargeFileUploadTask, MiddlewareFactory, OneDriveLargeFileUploadTask, PageIterator, Range, RedirectHandler, RedirectHandlerOptions, ResponseType, RetryHandler, RetryHandlerOptions, StreamUpload, TelemetryHandler, TelemetryHandlerOptions, UploadResult, getValidRangeSize };
